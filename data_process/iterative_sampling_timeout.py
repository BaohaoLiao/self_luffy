...
class CLIConfig:
    ...
    # scoring
    score_timeout: int = 30  # Timeout in seconds for each score computation
    score_batch_size: int = 50  # Restart subprocess after this many scores
...
def compute_score_worker(args):
    """Worker function that runs in subprocess."""
    model_output, ground_truth, timeout_score = args
...
def compute_scores_batch(
    responses, ground_truth, timeout_seconds=30, timeout_score=0.0
):
    """
    Compute scores for a batch of responses using multiprocessing with timeout.
    Restarts the process pool periodically to avoid accumulated state issues.
    """
    scores = []
    args_list = [(response, ground_truth, timeout_score) for response in responses]

    # Use multiprocessing pool with timeout
    with mp.Pool(processes=1) as pool:
        try:
            # Use map_async with timeout
            result = pool.map_async(compute_score_worker, args_list)
            scores = result.get(timeout=timeout_seconds * len(responses))
        except mp.TimeoutError:
            print(
                f"    WARNING: Batch scoring timed out after {timeout_seconds * len(responses)}s"
            )
            # Return 0.0 for all responses in this batch
...
def compute_all_scores(all_responses, ground_truths, batch_size=50, timeout_seconds=30):
    """
    Compute scores for all responses, processing in batches and restarting
    the subprocess periodically.
    """
    all_scores = []
    ...
    for i in range(len(all_responses)):
        ...
        # Process responses in batches, restarting subprocess for each batch
        scores = []
        for j in range(0, len(responses), batch_size):
            batch_responses = responses[j : j + batch_size]

            print(
                f"    Scoring responses {j + 1}-{min(j + batch_size, len(responses))} of {len(responses)} (problem {i + 1}/{len(all_responses)})"
            )

            batch_scores = compute_scores_batch(
                batch_responses, ground_truth, timeout_seconds=timeout_seconds
            )
...
def main(cli_config):
    ...
    print(f"[GPU {cli_config.local_idx}] Collecting responses...")
    all_responses = []
    ground_truths = []
    problems = []
    ...
    for i, output in enumerate(outputs):
        ...
        all_responses.append(new_responses)

    # Compute scores in batches with subprocess restarts
    print(
        f"[GPU {cli_config.local_idx}] Computing scores with periodic subprocess restarts..."
    )
    all_scores = compute_all_scores(
        all_responses,
        ground_truths,
        batch_size=cli_config.score_batch_size,
        timeout_seconds=cli_config.score_timeout,
    )
